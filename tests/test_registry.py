...
def _fit(columns=None):
    columns = columns or FEATURE_ORDER
...
class TestSkewTripwire:
    def test_manifest_order_differing_from_serving_is_refused(self, tmp_path):
        _save(tmp_path)
        path = tmp_path / "v1" / "manifest.json"
        data = json.loads(path.read_text())
        data["feature_order"] = list(reversed(FEATURE_ORDER))
        path.write_text(json.dumps(data))
        with pytest.raises(FeatureOrderMismatch, match="serving computes"):
